...
module.exports = function (/*globalMongoose*/) {
  ...
  m.init = (appLib) => {
    ...
    m.appLib.addRoute('get', `/get-drug-info-by-ndc-code/:ndc`, [m.getDrugInfoByNdcCode]);
    m.appLib.addRoute('get', `/get-drug-info/:id`, [m.getMedicationDocById]);
    m.appLib.addRoute('get', `/get-drug-info-by-match-predictive`, [m.getDrugByMatchPredictive]);
  ...
  m.getDrugByMatchPredictive = (req, res, next) => {
    ...
    const limit = Math.min(parseInt(req.query.limit) || DEFAULT_PAGE_SIZE, DEFAULT_PAGE_SIZE);
    const page = Math.max(parseInt(req.query.page), 1);

    const skip = (page - 1) * limit;

    const medicationMaster = m.appLib.db.model('medicationMaster');
    medicationMaster.find(condition).limit(limit).skip(skip).exec()
      .then((result) => {
  ...
  m.getMedicationDocById = (req, res, next) => {
    ...
    const medicationMaster = m.appLib.db.model('medicationMaster');
    const medicationsOpenFda = m.appLib.db.model('medicationsopenfda');

    let medicationMasterDoc;
    medicationMaster.findOne({_id: docId}).lean().exec()
      .then((result) => {
        if (!result) {
          return Promise.reject({code: 'notFound'});
        }
        medicationMasterDoc = result;
        return medicationsOpenFda.findOne({_id: medicationMasterDoc.openFdaData.id}).lean().exec();
      })
      .then((medicationsOpenFdaDoc) => {
        const packageLabels = medicationsOpenFdaDoc.rawData.package_label_principal_display_panel;
