module.exports = function (globalMongoose) {
  ...
  const getNotStartedQuestionnaries = (userId) => {
    let origAggregationPipeline = [
        ...
        {"$addFields": {isQuestionAnswered: {$in: ["$_id.questionaireId", "$answeredQuestionnaireIds"]}}},
    ...
    let hackedAggregationPipeline = [
        ...
        {"$addFields": {isQuestionAnswered: {$in: [{"$toString": "$_id.questionaireId"}, "$answeredQuestionnaireIds._id"]}}},
        {$match: {isQuestionAnswered: false}},
        {
            $group: {
                _id: '$_id.questionaireId',
    ...
    return mongoose.model('questionnaires').aggregate(hackedAggregationPipeline);
  };
  ...
  const getInProgressAndCompletedQuestionnaries = (userId) => {
            ...
            "$group": {
                ...
                "stats": {
                    "$push": {
                        ...
                        "status": {
                            "$ifNull": [
                                "$answersToQuestionnaires.status",
                                "Completed"
                            ]
                        }
                    }
    ...
    return mongoose.model('participants').aggregate(hackedAggregationPipeline).then(docs => {
      _.forEach(docs, doc => {
        const newStats = {};
  ...
  const getQuestionnaireSpentTimeChart = (userId) => {
    ...
    let hackedAggregationPipeline = [
        ...
        {"$addFields": {"questionnaireObjectId": { "$toObjectId": "$answersToQuestionnaires.questionnaireId._id"}}},
        {
            "$lookup": {
                "from": "questionnaires",
                "localField": "questionnaireObjectId",
  ...
  const getQuestionnaireDayChart = (days = 30, userId) => {
    ...
    return mongoose.model('participants').aggregate(hackedAggregationPipeline).then(doc => {
      _.forEach(doc, stat => {
  ...
  const getQuestionnairesCount = (userId) => { // TODO: scope to the current user only
    return mongoose.model('questionnaires').count({creator: userId});
  };

  const getAnswersCount = (userId) => { // TODO: scope to the current user only
    return mongoose.model('participants').aggregate([
        {
            "$project": {
                "answersToQuestionnaires": 1,
                "_id": 0
        ...
        { "$addFields": { "questionnaireObjectId": { "$toObjectId": "$answersToQuestionnaires.questionnaireId._id" }}},
        {$lookup: {from: "questionnaires", localField: "questionnaireObjectId", "foreignField": "_id", "as": "questionnaires"}},
  ...
  m.dashboard = (req, res, next) => {
    Promise.all([
      getQuestionnaireProgressChart(new ObjectID(req.user._id)),
      getQuestionnaireSpentTimeChart(new ObjectID(req.user._id)),
      getQuestionnaireDayChart(30, new ObjectID(req.user._id)),
      getQuestionnairesCount(new ObjectID(req.user._id)),
      getAnswersCount(new ObjectID(req.user._id)),
      getParticipantsCount(new ObjectID(req.user._id))
    ])
      .then(([
               questionnaireProgressChart, questionnaireSpentTimeChart, questionnaireDayChart,
               questionnairesCnt, answersCnt, participantsCnt
             ]) => {
        const dashboardData = {
