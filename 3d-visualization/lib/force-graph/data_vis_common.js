...
module.exports = {
  ...
  getDbData: async (appLib, req) => {
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            {
              $match: {
                $and: [modelToConditions.relationships, notHidden, { $expr: { $eq: ['$domain._id', '$$domainId'] } }],
              },
            },
          ],
        },
      ...
      { $unwind: '$relationship' },
      {
        $lookup: {
          ...
          pipeline: [
            {
              $match: {
                $and: [modelToConditions.entityTypes, notHidden, { $expr: { $eq: ['$_id', '$$domainTypeId'] } }],
              },
            },
          ],
        },
      ...
      { $unwind: '$range' },
      {
        $lookup: {
          ...
          pipeline: [
            {
              $match: {
                $and: [modelToConditions.entityTypes, notHidden, { $expr: { $eq: ['$_id', '$$rangeTypeId'] } }],
              },
            },
          ],
        },
      ...
      { $unwind: { path: '$range.type', preserveNullAndEmptyArrays: true } },
      {
        $lookup: {
          ...
          pipeline: [
            {
              $match: {
                $and: [modelToConditions.relationshipTypes, notHidden, { $expr: { $eq: ['$_id', '$$typeId'] } }],
              },
            },
          ],
        },
    ...
    const relationships = await appLib.db.collection('entities').aggregate(fullRelationshipsPipeline).toArray();
    ...
    const noRelationshipsPipeline = [
      {
        $match: {
          $and: [{ _id: { $nin: entityIdsInRelationships } }, modelToConditions.entities, notHidden],
        },
      },
      {
        $lookup: {
