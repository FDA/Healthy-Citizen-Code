...
async function parseSingleAlertPage (pageUrl) {
  ...
  try {
    const [, audience, issue, background, recommendation, posted] = articleText
      .match(/AUDIENCE:(.*)ISSUES?:(.*)BACKGROUND:(.*)RECOMMENDATIONS?:(.*)(\[(\d\d\/)?\d\d\/\d\d\d\d\ -.*])/);
    return {
      productName,
...
function upsertSafetyAlert (alert, dbCon, collectionName) {
  return dbCon.collection(collectionName).findOneAndReplace({productName: alert.productName}, alert, {upsert: true});
}
