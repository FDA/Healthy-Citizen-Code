;(function () {
  ...
  function AdpTablesActionsService() {
    function createActions(actions, tableIndex, data) {
      var filteredActions = {};

      _.each(actions.fields, function(action, key) {
      ...
      return _.map(filteredActions, function(action, name) {
        if (action.action.type === 'action') {
          return btnTpl(action, name, tableIndex, data);
        } else {
          return linkTpl(action, name, tableIndex, data)
        }
      }).join('\n');
    }
