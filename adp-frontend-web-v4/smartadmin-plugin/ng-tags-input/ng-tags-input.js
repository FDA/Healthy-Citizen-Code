    ...
    return {
      ...
      link: function(scope, element, attrs, ngModelCtrl) {
        var hotkeys = [KEYS.enter, KEYS.comma, KEYS.space, KEYS.backspace, KEYS.delete, KEYS.left, KEYS.right],
          ...
          setElementValidity,
          focusInput;
        var $tagList = element.find('.tag-list');

        if (options.sortable) {
          $tagList.sortable({
            update: function () {
              $tagList.find('[ng-bind="$getDisplayText()"]').each(function (index, item) {
                scope.tagList.items[index][options.displayProperty] = item.innerText;
              });
              events.trigger('tags-changed');
            }
          });
        ...
        events
          ...
          .on('tag-added tag-removed', function() {
            ...
            if (options.sortable) {
              $tagList.sortable('refresh');
            }
          })
          .on('tags-changed', function () {
            scope.tags = tagList.getItems();
          })
    ...
    '<div class="host" tabindex="-1" ng-click="eventHandlers.host.click()" ti-transclude-append>',
        '<div class="tags" ng-class="{focused: hasFocus}">',
          '<ul class="tag-list">',
            ...
            '</li>',
          '</ul>',
          '<input class="input" autocomplete="off" ng-model="newTag.text" ng-model-options="{getterSetter: true}" ng-keydown="eventHandlers.input.keydown($event)" ng-focus="eventHandlers.input.focus($event)" ng-blur="eventHandlers.input.blur($event)" ng-paste="eventHandlers.input.paste($event)" ng-trim="false" ng-class="{\'invalid-tag\': newTag.invalid}" ng-disabled="disabled" ti-bind-attrs="{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex, spellcheck: options.spellcheck}" ti-autosize></div>',
      '</div>'
      ].join('')
    );
