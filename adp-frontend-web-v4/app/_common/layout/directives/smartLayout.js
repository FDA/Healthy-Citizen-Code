...
angular.module('SmartAdmin.Layout').directive('smartLayout', function (
  $rootScope,
  $timeout,
  $interval,
  $transitions,
  ...
  AdpLayoutConfigService
    ...
    (function cacheElements() {
        ...
        if (_.every([$navigation, $menu, $ribbon, $footer], function ($it) {
            ...
            $timeout(cacheElements, 0);
        }
    })();

    (function applyConfigSkin(){
      $body.removeClass(_.map(window.appConfig.skins, 'name').join(' '));
      $body.addClass(AdpLayoutConfigService.getSkin());
    })();

  (function defineLayoutParams() {
    var classes = AdpLayoutConfigService.layoutClass();
    var bg = AdpLayoutConfigService.get('background');
