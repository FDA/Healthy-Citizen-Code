(function ()
{
    ...
    apiService.$inject = ['$resource', 'CONSTANTS', '$http', 'hcSessionService']
    /** @ngInject */
    function apiService($resource, CONSTANTS, $http, hcSessionService)
    {
        /**
        ...
        // Base Url
        api.baseUrl = CONSTANTS.apiUrl;

        // User
        api.user = $resource(api.baseUrl + 'user', {}, {
            'get' : {
                method: 'GET'
            },
            'update' : {
        ...
        // settings
        api.settings = {
            list       : $resource(api.baseUrl + 'settings'),
            single : $resource(api.baseUrl + 'settings/:id', {id: '@id'}, {
                'get' : {
                    method: 'GET'
                },
                'update' : {
        ...
        // phiData
        api.phiData = $resource(api.baseUrl + 'phidata', {}, {
            'get' : {
                method: 'GET'
            },
            'update' : {
        ...
        // medication
        api.medication = {
            list       : $resource(api.baseUrl + 'medication'),
            single : $resource(api.baseUrl + 'medication/:id', {id: '@id'}, {
                'get' : {
                    method: 'GET'
                },
                'update' : {
        ...
        // Hopsitals
        api.hospitals = $resource(api.baseUrl + 'hospitals');

        // Sync User
        api.syncUser = $resource(api.baseUrl + 'sync');

        // Alerts
        api.alerts = $resource(api.baseUrl + 'alerts');


        // Passcode verification
        api.sendPasscode = $resource(api.baseUrl + 'email');
        api.verifyPasscode = $resource(api.baseUrl + 'passcode');

        api.auth = {
            sendPasscode: $resource(api.baseUrl + 'auth/email'),
            verifyPasscode: $resource(api.baseUrl + 'auth/passcode'),
            settings: $resource(api.baseUrl + 'auth/settings'),
            twoFactorAuthVerify: $resource(api.baseUrl + 'auth/2factorauth/verify'),
            qrcode: $resource(api.baseUrl + 'auth/qrcode')
        };

        // Schemas
        api.schema = {
            user: $resource(api.baseUrl + 'schema/user'),
            settings: $resource(api.baseUrl + 'schema/settings'),
            phiData: $resource(api.baseUrl + 'schema/phidata'),
            piiData: $resource(api.baseUrl + 'schema/piidata'),
            medication: $resource(api.baseUrl + 'schema/medication')
        };
