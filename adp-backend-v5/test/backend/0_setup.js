 ...
 */
// const dotenv = require('dotenv').load({path: '.env'}); // Please do not comment out this line, it will break util
/*
...
global.mutil = require('../lib/model')(global.appLib);
global.mainController = require("../lib/default-controller")(global.appLib);

global.isDateString = function (str) {
...
global.checkForEqualityConsideringInjectedFields = function (data, sample) {
  const objDiff = diffObjects(data, sample);
  // check synthesized updatedAt and createdAt fields
  objDiff.missing_from_second.forEach(path => {
    assert(path.endsWith('updatedAt') || path.endsWith('createdAt'));
