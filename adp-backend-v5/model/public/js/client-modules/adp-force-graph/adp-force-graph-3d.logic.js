  ...
  function forceGraphControllerLogic() {
    return function (
      ...
      var configFieldsLimits = {
        hlColorIntensity: { min: 0, max: 100 },
        hlDimIntensity: { min: 0, max: 100 },
      ...
      vm.saveConfig = function () {
        ...
        $http
          .post(APP_CONFIG.apiUrl + '/graphql', {
            query: 'mutation m( $config:JSON){ userConfigUpsertOne(record: { fg3d_config: $config }) { fg3d_config } }',
            variables: {
        ...
        if (geometry) {
          ...
          if (!isLayerVisible(node)) {
            ...
            var color = isSelected
              ? SELECTED_COLOR
              : isLit
              ? mixColors(node.col, HIGHLIGHT_COLOR, vm.config.hlColorIntensity / 100)
              : isFiltered
              ? mixColors(node.col, FILTERED_COLOR, vm.config.hlColorIntensity / 100)
              : (node.col || DEFAULT_COLOR).toLowerCase();

            var opacity =
              isMarked || nothingIsMarked
                ? NODE_COMMON_OPACITY
                : NODE_COMMON_OPACITY * (1 - vm.config.hlDimIntensity / 100);
      ...
      function linkCustomPositionUpdate(sprite, p) {
        ...
        var midPos = {
          x: p.start.x + (p.end.x - p.start.x) / 2,
          y: p.start.y + (p.end.y - p.start.y) / 2,
          z: p.start.z + (p.end.z - p.start.z) / 2,
        };
      ...
      function linkColor(link) {
        return link.isSelected
          ? SELECTED_COLOR
          : isLinkFiltered(link)
          ? mixColors(link.col, FILTERED_COLOR, vm.config.hlColorIntensity / 100)
          : isLinkLit(link)
          ? mixColors(link.col, HIGHLIGHT_COLOR, vm.config.hlColorIntensity / 100)
          : link.col;
      }
    ...
    function mixColors(col1, col2, mix) {
      var col = Color(col1);

      return col.mix(Color(col2), mix || 0.5).hex();
    }
