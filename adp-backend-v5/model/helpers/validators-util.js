...
module.exports = function (appLib) {
   ...
   * @returns {string|XML}
   */
  m.replaceErrorTemplatePlaceholders = function (modelName, spec, val, data, lodashDataPath, appModelPart, templateName) {
    if (!templateName) {
   ...
   * @returns {*}
   */
  m.getCastedValue = function (val, appModelPart) {
  ...
  m.levelUp = function (str) {
    var pos = str.lastIndexOf('.');
    return pos > -1 ? str.substr(0, pos + 1) : '';
  };

  m.getArgumentValue = function (spec, lodashSpecPath, data, lodashDataPath, appModelPart, noCasting) {
   ...
   */
  m.getValue = function (data, appModelPart, lodashPath, noCasting) {
   ...
   * @param date
   */
  m.getDatePartValue = function (date) {
    var d = new Date(date);
    d.setHours(0, 0, 0, 0);
    return d.getTime();
  };
   ...
   * @returns {string}
   */
  m.getDatePartString = function (date) {
    var d = new Date(date);
