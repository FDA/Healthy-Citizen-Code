...
function cpFonts(destBasePath) {
  var fontsPath = [
    ...
    path.join(conf.moduleDir, '/font-awesome/fonts/**'),
  ];

  return gulp.src(fontsPath)
    .pipe(gulp.dest(path.join(destBasePath, 'assets/fonts')));
}
...
function cpDxIcons(destBasePath) {
  ...
  return gulp.src(fonts)
    .pipe(gulp.dest(path.join(destBasePath, 'styles/icons')));
}
...
function cpDxFonts(destBasePath) {
  ...
  return gulp.src(fonts)
    .pipe(gulp.dest(path.join(destBasePath, 'styles/fonts')));
}
...
function cpJson(destPath) {
  return gulp.src([
    path.join(conf.paths.tmp, 'manifest.json'),
    'api/**/*'
  ])
...
function cpWorkers() {
  return gulp.src([
    ...
    .pipe(gulp.dest(path.join(conf.paths.distTmp, '/workers')))
}
