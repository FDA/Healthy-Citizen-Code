...
class DashboardItemsView extends Component {
  ...
  static navigationOptions = ({navigation, screenProps, navigationOptions}) => ({
    title: navigation.state.params.title || 'Dashboard',
    headerRight: (
      typeof navigation.state.params.goToEditScreen === 'function' ? (
        <Button
          onPress={() => navigation.state.params.goToEditScreen()}
          transparent
        >
          <Icon name='add'/>
        </Button>
  ...
  componentWillMount() {
    const {navigation, dashboards} = this.props;
    ...
    navigation.setParams({
      title: _.get(dashboards, [contentIndex, 'fullName'], ''),
      goToEditScreen: () => {
        this.goTo(dashboards[this.state.contentIndex].path, 'edit');
      }
    });
  ...
  _onMomentumContentScrollEnd(newIndex, withoutAnimation) {
    ...
    navigation.setParams({
      title: _.get(dashboards, [newIndex, 'fullName'], '')
    });
        ...
        <Container>
            ...
            >
              {
                dashboards.map((item, index) => (
                  ...
                  >
                    {this.getDashboardContent(item)}
                    {
                      isBottomsArrowShowed ?
                        <View
                          style={[styles.tileArrow, {
                            width: width / dashboardButtonsGroupLength,
                            left: (width / dashboardButtonsGroupLength) * (index % dashboardButtonsGroupLength)
                          }]}
                        >
                          <Triangle
                            width={15}
          ...
          >
            <Swiper
              ...
              prevButton={<Triangle width={13} height={15} color='#BBBBBB' direction='left'
                                    style={styles.swiperButtonLeft}/>}
              nextButton={<Triangle width={13} height={15} color='#BBBBBB' direction='right'
                                    style={styles.swiperButtonRight}/>}
  ...
  getDashboardContent(item) {
    const isHCDashboard = item.template.startsWith('<adp-chart');
    if (isHCDashboard) {
      return <View
        style={[styles.highchartsContent, {borderColor: item.color}]}
      >
        <TemplateGenerator
          item={item}
    ...
    >
      <ScrollView
        ref={(ref) => {
          this.contentScrollViews[item.id] = ref;
        }}
