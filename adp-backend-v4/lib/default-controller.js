...
module.exports = function () {
    ...
    m.getRootJson = (req, res, next) => {
        res.json({success: true, message: `HC Backend V4 is working correctly`});
        next();
    };
    ...
    m.getLookupTableJson = (req, res, next) => {
        ...
        let schemaLookup = (lookup, req, res, next) => {
        ...
        let subSchemaLookup = (lookup, req, res, next) => {
            // TODO: support admin function when admin can lookup any user's
            let tableParts = lookup.table.split("/"); // starts with "/"
            ...
            if(tableParts.length != 4 ) {
                ...
                if (!model) {
                    ...
                    let recordId = req.user[tableParts[2].substring(1)];
                    model.findById(recordId).lean().exec((err, data) => {
                        if(err) {
                            ...
                            let records = data[tableParts[3]];
                            let labelName = _.get(lookup, 'label', "_id");
                            let foreignKeyName = _.get(lookup, 'foreignKey', "_id");
                            let ret = _(records).map((row) => {
        ...
        if(lookup.table.startsWith("/")) { // this si a subschema lookup
    ...
    m.getItem = (req, res, next) => {
        getElement(req, res, next, (err, part) => {
            if (err) {
                return m.error(req, res, next, err, "Internal error: unable to find requested element" );
            } else {
    ...
    m.putItem = (req, res, next) => {
        ...
        getElement(req, res, next, (err, part, data, parent, parentKey, model, mongoConditions) => {
            ...
            if (err) {
                return m.error(req, res, next, err, "Internal error: unable to update this item");
    ...
    m.postItem = (req, res, next) => {
        getElement(req, res, next, (err, part, data, parent, parentKey, model, mongoConditions) => {
            if (err) {
                return m.error(req, res, next, err, "Internal error: unable to create this item");
            } else {
    ...
    m.getClientSideCodeForFormRenderers = (req, res, next) => {
        butil.loadFormRenderers();
        let body = getJsPrefix('FormRenderers') + getJsObjectString(appModelHelpers.FormRenderers) + '};';
        sendJavascript(res, body, next);
    };
    ...
    m.getClientSideCodeForRenderers = (req, res, next) => {
        butil.loadRenderers();
        let body = getJsPrefix('Renderers') + getJsObjectString(appModelHelpers.Renderers) + '};';
        sendJavascript(res, body, next);
    };
    ...
    m.getClientSideCodeForCustomActions = (req, res, next) => {
        ...
        let body = getJsPrefix('CustomActions') + getJsObjectString(appModelHelpers.CustomActions) + '};';
        sendJavascript(res, body, next);
    };
    ...
    m.getAppModelCodeStr = () => {
        ...
        return "" +
            ...
            getJsPrefix('Renderers') + getJsObjectString(appModelHelpers.Renderers) + '};' +
            getJsPrefix('CustomActions') + getJsObjectString(appModelHelpers.CustomActions) + '};' +
            getJsPrefix('FormRenderers') + getJsObjectString(appModelHelpers.FormRenderers) + '};' +
    ...
    m.getAppModelCode = (req, res, next) => {
        sendJavascript(res, m.getAppModelCodeStr(), next);
    };
    ...
    m.getMinifiedAppModelCode = (req, res, next) => {
        let miniJs = uglify.minify(m.getAppModelCodeStr());
        if(miniJs.error) {
